...
public class SimpleTap : MonoBehaviour {
	...
	private void LaunchBomb(){
		...
		GameObject bomb = Instantiate(Resources.Load("Button the Penguin") as GameObject);
		bomb.transform.localScale = Vector3.one * 0.35f;
		bomb.transform.position = Camera.main.transform.position;
		bomb.transform.rotation = Camera.main.transform.localRotation;
		bomb.AddComponent<Rigidbody>();
		bomb.GetComponent<Rigidbody> ().mass = 60;
		bomb.GetComponent<Rigidbody> ().AddRelativeForce (500, 5, 30000);
		bomb.GetComponent<Rigidbody> ().AddRelativeTorque (50, 500, 500);
